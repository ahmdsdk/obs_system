...
export default function MiniDrawer({ userData, logOut, panelItems, setPanelItems, components }) {
  ...
  const onListItemClick = (e, item, index) => {
    if (item.path === "logout") {
      logOut(e);
    } else {
      let pI = [...panelItems];
      pI.forEach(item => item.isSelected = false);
      pI[index].isSelected = true;
      setPanelItems(pI);
        ...
        <List>
          {panelItems.map((item, index) => (
            <ListItem key={item.text} disablePadding sx={{ display: 'block' }}>
              <ListItemButton
                onClick={(e) => onListItemClick(e, item, index)}
                onMouseOver={() => {
                  let pI = [...panelItems];
                  pI[index].isHovered = true;
                  setPanelItems(pI);
                }}
                onMouseLeave={() => {
                  let pI = [...panelItems];
                  pI[index].isHovered = pI[index].isSelected && false;
                  setPanelItems(pI);
                }}
                sx={{
                  minHeight: 48,
